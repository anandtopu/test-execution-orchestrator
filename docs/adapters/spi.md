...
`ASTSignature` is an optional fifth component: a stable hash of the test's *normalized body* (not its location). It makes identity survive file renames/moves (path changes, signature doesn't → history preserved) and split when the body materially changes (new assertions / restructured logic → fresh identity, fresh flake history). It's normally computed during `Discover` and attached to each `TestEntry.ASTSignature` (the jest adapter, which can't enumerate tests without running, attaches it at `Execute` instead — see below); the worker carries it through both claim paths (Postgres SKIP-LOCKED and NATS `DispatchTest.ASTSignature`) and migration 006 persists it to `teo.tests.ast_signature`.
...
- **`pkg/adapter/jest`** runs an embedded Node helper (`astsig.go`) that parses each test file with `@babel/parser` (a transitive dependency of `babel-jest`, so present in virtually every Jest project; resolved from the project under test) and hashes the normalized `it()`/`test()` callback body. Because Jest can't enumerate test blocks without running (see below), the signature is attached at **`Execute`** time in `parseReport` — keyed by the same `describe > … > title` string the report yields — rather than at `Discover`. Dynamic titles (`it.each`, interpolated/computed names) can't be matched to a report `Name`, so they keep an empty signature (path+name+params identity, the v1.0 behavior). If `@babel/parser` or `node` is unavailable the helper degrades to empty signatures. S-14-02 AC3 (v1.5).
