...
**Last updated:** 2026-05-20 (⚠ **backlog reconciliation** found **12 items marked ✅ here that were not wired in code** — see [Backlog reconciliation](#backlog-reconciliation--verified-incomplete-items-2026-05-20) below. **Then fixed 5 of them: #1 `teo replay`, #3 Go AST fingerprint, #4 Python AST fingerprint, #5 log-tail viewer, #6 OIDC sign-in** (+ added `teo discover`, migration 006). **7 remain** — only 1 functional (#2 S3 plan archive, partial); the rest polish/test-debt. Prior 2026-05-05 entry: Gap #5 GraphQL authz gate landed; `internal/nats` unit-tested.)
...
**Release impact:** items **1, 3, 4, 5, 6 are now resolved** (2026-05-20). The only remaining functional item is **2** (plan archived to S3) — a partial: `teo replay` works off the Postgres-persisted plan, but the S3 `runs/<id>/plan.json` archive named in the AC is still absent. The rest are polish (7, 8, 9, 11) or test debt (10, 12).
...
> 🟡 rows below have one or more **open acceptance criteria** from the reconciliation table above (the Notes describe delivered scope; the 🟡 flags what's still missing): **E-05** (#2 S3 plan archive), **E-09** (#11 run-list virtual scroll), **E-12** (#7 calibration overlay), **E-15** (#8 Mermaid issue history). E-02/E-04 keep ✅ with open test follow-ups (#10 load test, #12 leader-election test); E-07 keeps ✅ with a minor backfill follow-up (#9).
...
> ⚠ Reconciliation update (2026-05-20): **FR-703–705** (#5 log-tail viewer), **FR-801** (#6 OIDC sign-in flow), and the Go/Python **AST-signature fingerprints** (#3/#4) are now wired; a new `teo discover` CLI closes **S-06-01 AC1**. **FR-304** is partially resolved — `teo replay` (#1) landed, but the plan is archived in Postgres, not the S3 `runs/<id>/plan.json` path the AC names (#2). Treat the [Backlog reconciliation](#backlog-reconciliation--verified-incomplete-items-2026-05-20) table as ground truth where rows disagree.
...
**Resolved 2026-05-20:** #1 `teo replay` CLI, #3 Go AST fingerprint, #4 Python AST fingerprint, #5 log-tail viewer, #6 OIDC sign-in flow (see the reconciliation table). Also added `teo discover` (closes S-06-01 AC1).

**Remaining functional item:**
1. Plan archived to S3 `runs/<id>/plan.json` (#2, S-05-04 AC1) — `teo replay` already verifies determinism off the Postgres-persisted plan; the S3 archive is the remaining piece of FR-304.

**Polish / test-debt (defer-to-v1.1 candidates):** #7 calibration overlay, #8 Mermaid issue history, #9 failure backfill, #11 run-list virtual scroll; #10 ClickHouse load test, #12 leader-election chaos test.
