...
| 4 | Python AST-signature fingerprint | S-06-01 / T-06-01-03 | ✅ **done** (2026-05-20) | [`pkg/adapter/pytest/astsig.go`](pkg/adapter/pytest/astsig.go) runs an embedded `ast`-module helper to hash each test's body (module funcs + `Test*` class methods). Unit-tested end-to-end against a real Python interpreter |
...
**Release impact:** items **1, 2, 3, 4, 5, 6 are now resolved** (2026-05-20) — every functional gap from the audit is closed. The remaining 6 are all polish (7, 8, 9, 11) or test debt (10, 12); none block a v1.0 tag on functional grounds.
...
> 🟡 rows below have one or more **open acceptance criteria** from the reconciliation table above (the Notes describe delivered scope; the 🟡 flags what's still missing) — all polish at this point: **E-09** (#11 run-list virtual scroll), **E-12** (#7 calibration overlay), **E-15** (#8 Mermaid issue history). E-02/E-04 keep ✅ with open test follow-ups (#10 load test, #12 leader-election test); E-07 keeps ✅ with a minor backfill follow-up (#9).
...
**Remaining — all polish / test-debt (defer-to-v1.1 candidates):** #7 calibration overlay, #8 Mermaid issue history, #9 failure backfill, #11 run-list virtual scroll; #10 ClickHouse load test, #12 leader-election chaos test.
